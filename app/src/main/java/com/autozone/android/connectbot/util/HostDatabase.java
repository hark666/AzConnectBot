...
import com.autozone.android.annotation.KeepForTesting;
import com.autozone.android.connectbot.bean.HostBean;
import com.autozone.android.connectbot.bean.PortForwardBean;
import com.autozone.android.connectbot.data.ColorStorage;
...
public class HostDatabase extends RobustSQLiteOpenHelper implements HostStorage, ColorStorage {
	...
	public final static String FIELD_HOST_APPNAME = "appname";
	public final static String FIELD_HOST_ENVIROMENT = "enviroment";
	...
	public static final String TABLE_HOSTS_COLUMNS = "_id INTEGER PRIMARY KEY, "
			...
			+ FIELD_HOST_APPNAME + " TEXT, "
			+ FIELD_HOST_ENVIROMENT + " TEXT, "
	...
	/**
	 * Return a cursor that contains all the apps.
	 */
	@Override
	public List<String> getApps() {
		String[] columns = new String[1];
		columns[0] = FIELD_HOST_APPNAME;
		Cursor c = mDb.query(TABLE_HOSTS, columns, null, null, FIELD_HOST_APPNAME, null, FIELD_HOST_APPNAME + " ASC");

		ArrayList<String> apps = new ArrayList<>();
		final int COL_APPNAME = c.getColumnIndexOrThrow(FIELD_HOST_APPNAME);
		while (c.moveToNext()) {
			if(!apps.contains(c.getString(COL_APPNAME))){
				apps.add(c.getString(COL_APPNAME));
			}
		}
	...
	public List<String> getEnviroments() {
		String[] columns = new String[1];
		columns[0] = FIELD_HOST_ENVIROMENT;
		Cursor c = mDb.query(TABLE_HOSTS, columns, null, null, FIELD_HOST_ENVIROMENT, null, FIELD_HOST_ENVIROMENT + " ASC");

		ArrayList<String> enviroments = new ArrayList<>();
		final int COL_ENVIROMENT = c.getColumnIndexOrThrow(FIELD_HOST_ENVIROMENT);
		while (c.moveToNext()) {
			if(!enviroments.contains(c.getString(COL_ENVIROMENT))){
				enviroments.add(c.getString(COL_ENVIROMENT));
			}
		}
	...
	@Override
	public List<String> getEnviroments(String appName) {
		String[] columns = new String[1];
		columns[0] = FIELD_HOST_ENVIROMENT;

		String[] sectionArgs = new String[1];
		sectionArgs[0] = appName;
		Cursor c = mDb.query(TABLE_HOSTS, columns, FIELD_HOST_APPNAME + " = ?", sectionArgs, FIELD_HOST_ENVIROMENT, null, FIELD_HOST_ENVIROMENT + " ASC");
	...
	@Override
	public List<HostBean> getHosts(String appName, String enviroments, boolean sortColors) {
		List<HostBean> hosts;
		String sortField = sortColors ? FIELD_HOST_COLOR : FIELD_HOST_NICKNAME;

		String[] sectionArgs = new String[2];
		sectionArgs[0] = appName;
		sectionArgs[1] = enviroments;

		Cursor c = mDb.query(TABLE_HOSTS, null, FIELD_HOST_APPNAME + " = ? AND " + FIELD_HOST_ENVIROMENT + " = ?", sectionArgs, null, null, sortField + " ASC");
	...
	private List<HostBean> createHostBeans(Cursor c) {
		...
		final int COL_ID = c.getColumnIndexOrThrow("_id"),
			...
			COL_APPNAME = c.getColumnIndexOrThrow(FIELD_HOST_APPNAME),
			COL_ENVIROMENT = c.getColumnIndexOrThrow(FIELD_HOST_ENVIROMENT),
		...
		while (c.moveToNext()) {
			...
			host.setAppname(c.getString(COL_APPNAME));
			host.setEnviroment(c.getString(COL_ENVIROMENT));
