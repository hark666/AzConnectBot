...
package com.autozone.android.connectbot.transport;
...
import com.autozone.android.connectbot.bean.PubkeyBean;
...
import com.autozone.android.connectbot.service.TerminalManager.KeyHolder;
import com.autozone.android.connectbot.util.HostDatabase;
import com.autozone.android.connectbot.util.PubkeyDatabase;
import com.autozone.android.connectbot.util.PubkeyUtils;
...
public class SSH extends AbsTransport implements ConnectionMonitor, InteractiveCallback, AuthAgentCallback {
	...
	private void authenticate() {
		String AzUserName = this.ignitionID + "@" + this.host.getUsername() + "@" + this.host.getHostname();

		try {
			if (connection.authenticateWithNone(AzUserName)) {
				finishConnection();
				return;
			}
		} catch (Exception e) {
		...
		try {
			...
			if (!pubkeysExhausted &&
			...
			} else if (connection.isAuthMethodAvailable(AzUserName, AUTH_PASSWORD)) {
				...
				if (password != null
						&& connection.authenticateWithPassword(AzUserName, password)) {
					finishConnection();
				} else {
	...
	public void connect() {
		...
		connection = new Connection("cyberark-ssh.autozone.com", host.getPort());
